...
# 1.确定原图像数据集路径
dataset_dir = "C:\document\python\pythonDemo\pytorchTest\data\Rice_Image_Dataset"  ##原始数据集路径
# 2.确定数据集划分后保存的路径
...
for root, dirs, files in os.walk(dataset_dir):
    for sub_dir in dirs: # 遍历0，1，2，3，4，5...9文件夹
        ...
        train_point = int(img_count * train_pct)  # 训练数据数量
        valid_point = int(img_count * (train_pct + valid_pct))  # 训练+验证数据数量
